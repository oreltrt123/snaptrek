...
import { X, Search, UserPlus, Check, XIcon, Users, AlertTriangle, RefreshCw } from "lucide-react"
...
export function UserSearchSidebar({ isOpen, onClose, lobbyId, availablePosition }: UserSearchSidebarProps) {
  ...
  const [activeTab, setActiveTab] = useState("all") // Default to all users tab
  ...
  const fetchAllUsers = async () => {
    ...
    try {
      console.log(`Fetching all users (attempt ${retryCount + 1})...`)
